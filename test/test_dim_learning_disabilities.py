from utils.dim_learning_disabilities import dim_learning_disabilities
import pytest
import pandas as pd
...
19,64,Medium,Medium,No,8,59,Low,No,2,Medium,Medium,Private,Negative,4,Yes,College,Moderate,Female,61"""
    return pd.read_csv(StringIO(csv_data))


class TestUnit:
    def test_handle_wrong_input(self, create_df, caplog):
        response = dim_learning_disabilities("hello")
    ...
    def test_return_new_df(self, create_df):
        response = dim_learning_disabilities(create_df)

        assert response is not create_df
    
    def test_logs_error(self, create_df,caplog):
        df = pd.DataFrame({"SomeColumn": [1, 2, 3]})
        response = dim_learning_disabilities(df)

        assert response is None
        assert "error while transforming unique learning disabilities" in caplog.text
    ...
    def test_return_correct_values(self,create_df ):
        ...
        assert set(response["learning_disabilities"]).issubset({"Yes","No"})


    def test_output_column_name(self, create_df):
        result = dim_learning_disabilities(create_df)

        assert list(result.columns) == ["learning_disabilities"]
