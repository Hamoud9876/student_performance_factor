from utils.dim_access_to_resources import dim_access_to_resources
import pytest
import pandas as pd
...
class TestUnit:
    def test_handle_wrong_input(self, create_df, caplog):
        response = dim_access_to_resources("hello")
    ...
    def test_return_new_df(self, create_df):
        response = dim_access_to_resources(create_df)

        assert response is not create_df
    
    def test_logs_error(self, create_df,caplog):
        df = pd.DataFrame({"SomeColumn": [1, 2, 3]})
        response = dim_access_to_resources(df)

        assert response is None
        assert "error while transforming unique access to resources" in caplog.text
    ...
    def test_return_correct_values(self,create_df ):
        ...
        assert set(response["access_to_resources"]).issubset({"High", "Medium", "Low"})


    def test_output_column_name(self, create_df):
        result = dim_access_to_resources(create_df)

        assert list(result.columns) == ["access_to_resources"]
