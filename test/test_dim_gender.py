from utils.dim_gender import dim_gender
import pytest
import pandas as pd
...
class TestIntigration:
    ...
    def test_return_new_df(self, create_df):
        response = dim_gender(create_df)

        assert response is not create_df

    
    def test_missing_gender_column(self, create_df,caplog):
        df = pd.DataFrame({"SomeColumn": [1, 2, 3]})
        response = dim_gender(df)
    ...
    def test_return_correct_values(self,create_df ):
        ...
        assert set(response["gender"]) == {"Male", "Female"}


    def test_output_column_name(self, create_df):
        result = dim_gender(create_df)

        assert list(result.columns) == ["gender"]