from utils.dim_peer_influance import dim_peer_influance
import pytest
import pandas as pd
...
class TestUnit:
    def test_handle_wrong_input(self, create_df, caplog):
        response = dim_peer_influance("hello")
    ...
    def test_return_new_df(self, create_df):
        response = dim_peer_influance(create_df)

        assert response is not create_df
    
    def test_logs_error(self, create_df,caplog):
        df = pd.DataFrame({"SomeColumn": [1, 2, 3]})
        response = dim_peer_influance(df)

        assert response is None
        assert "error while transforming unique peer influance" in caplog.text
    ...
    def test_return_correct_values(self,create_df ):
        ...
        assert set(response["peer_influance"]).issubset({"Positive","Negative"})


    def test_output_column_name(self, create_df):
        result = dim_peer_influance(create_df)

        assert list(result.columns) == ["peer_influance"]
